...
def Set_Servo_x_angle(temp):
   if (temp > -50 and temp <46) :
      Servo_x.angle(temp,0)
   elif temp < -50:
      Servo_x.angle(-50,100)
   else:
      Servo_x.angle(50,100)

def Set_Servo_y_angle(temp):
   if (temp > -46 and temp <46) :
      Servo_y.angle(temp,100)
   elif temp < -46:
      Servo_y.angle(-46,100)
   else:
      Servo_y.angle(46,100)
...
class PID_Inc:
    ...
    #pid 设置
Pid_x = PID_Inc(0.2, 0, 0.1)
Pid_y = PID_Inc(0.2, 0, 0.1)
cnt =0
while(flag_dw):
    clock.tick()
    ...
    img.draw_cross(300,300,size=30, color=(255,255,255))
